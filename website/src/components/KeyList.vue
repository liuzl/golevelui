<template>
  <div class="key-list">
    <el-row class="filter-header">
      ...
      <el-col :span="2">
        <el-button @click="handleNew" type="primary" icon="el-icon-plus" circle></el-button>
      </el-col>
      <el-col :span="2">
...
<script lang="ts">
  ...
  export default class List extends Vue {
    ...
    handleItemClick(row: Item, isNew = false) {
      ...
      if (isNew) {
        this.currentValue = ""
        Message.info("请输入Value并点击更新来完成创建")
        return
      }
    ...
    handleNew() {
      MessageBox.prompt('请输入新的 Key', '新增', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(({ value }) => {
        if (value) {
          const newKey = { keyName: value }
          this.data.unshift(newKey) // Add to the top of the list
          this.count++
          this.handleItemClick(newKey, true) // Select it as a new key
        }
      }).catch(() => {
        Message.info('已取消新增');
      });
